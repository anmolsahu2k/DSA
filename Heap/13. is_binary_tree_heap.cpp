...
class Solution {
  public:
    bool isComplete(Node* root, int index, int no_of_nodes)
    {
        if(root==NULL)
            return true;
        if(index>=no_of_nodes)
            return false;
        return isComplete(root->left, 2*index+1,no_of_nodes)&&
                isComplete(root->right,2*index+2,no_of_nodes);
    }
    ...
    bool isHeap(struct Node* tree) {
        ...
        return isComplete(tree,0,n)&&isMaxHeap(tree);
    }
};
