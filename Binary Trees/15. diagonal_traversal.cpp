...
void diagonalTraversal(Node* root, int d, map<int, vector<int>> &mp)
{
    if(root==NULL)
        return;
    //d is same for nodes in each diagonal
    //push the nodes in map with d key
    mp[d].push_back(root->data);
    //for left node, increment d to distinguish diagonals
    diagonalTraversal(root->left, d+1,mp);
    //for right node, d remains same since diagonal is to the right
